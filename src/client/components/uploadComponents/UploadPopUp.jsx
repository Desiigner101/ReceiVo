...
const UploadPopUp = ({ onClose, onAddReceipt }) => {
  ...
  const handleSubmit = () => {
    ...
    const newReceipt = {
      ...
      currency,
      invoiceNumber,
      warranty: isElectronics ? 'Active' : 'None',
      claimTypes,
      productName: isElectronics ? productName : null,
      brand: isElectronics ? finalBrand : null,
      serialNumber: isElectronics ? serialNumber : null,
      warrantyProvider: isElectronics ? warrantyProvider : null,
      warrantyDuration: isElectronics ? warrantyDuration : null,
      customDuration: warrantyDuration === 'Custom' ? `${customDurationValue} ${customDurationUnit}` : null,
      warrantyEndDate: isElectronics ? warrantyEndDate : null,
      tags: selectedTags,
      notes,
      notificationPrefs,
      image: files.length > 0 ? files[0].name : null,
    };
